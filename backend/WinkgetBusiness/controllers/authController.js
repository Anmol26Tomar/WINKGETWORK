...
const signupVendor = async (req, res) => {
  try {
    ...
    console.log('🔍 Vendor signup attempt:', { ownerName, ownerEmail, shopName });
    console.log('📝 Full request body:', JSON.stringify(req.body, null, 2));
    
    if (!ownerName || !ownerEmail || !password || !shopName) {
      return res.status(400).json({ message: 'Missing required fields: ownerName, ownerEmail, password, shopName' });
    }
    ...
    console.log('📧 Existing email result:', existingEmail ? 'FOUND' : 'NOT FOUND');
    console.log('🏪 Existing shop name result:', existingShopName ? 'FOUND' : 'NOT FOUND');
    ...
    if (err.code === 11000) {
      ...
      const field = err.keyPattern?.ownerEmail ? 'ownerEmail' : 
                   err.keyPattern?.shopName ? 'shopName' :
                   err.keyPattern?.email ? 'ownerEmail' :
                   err.keyPattern?.storeName ? 'shopName' : 'unknown';
      
      return res.status(409).json({ 
        message: `${field === 'ownerEmail' ? 'Email' : 'Shop name'} already exists`, 
        field,
        duplicateValue: err.keyValue?.[field] || err.keyValue?.ownerEmail || err.keyValue?.shopName
      });
    }
