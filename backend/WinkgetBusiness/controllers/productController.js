...
const getProducts = async (req, res) => {
  try {
    ...
    if (secondarySubcategory)
      filter.secondarySubcategory = secondarySubcategory;
    ...
    const products = await Product.find(filter)
      .populate(
        "vendorRef",
        "shopName ownerName averageRating businessProfilePic"
      )
      .sort(sortOptions)
      .skip(skip)
...
const getProductById = async (req, res) => {
  try {
    const product = await Product.findById(req.params.id)
      .populate(
        "vendorRef",
        "shopName ownerName aboutBusiness averageRating businessProfilePic socialLinks websiteLink"
      )
...
const createProduct = async (req, res) => {
  try {
    ...
    const vendorRef =
      req.user.role === "vendor" ? req.user.id : payload.vendorRef;
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (
      ...
      product.vendorRef.toString() !== req.user.id
    ) {
...
// Public: Get products by vendorRef (shows products belonging to a vendorRef)
            ...
            $push: {
              ...
              secondarySubcategories: {
                $cond: [
                  ...
                  [
                    {
                      secondarySubcategory: "$_id.secondarySubcategory",
                      count: "$count",
                    },
                  ],
                  [],
