...
router.get("/orders/nearby", auth, async (req, res) => {
  try {
    ...
    console.log(
      `Fetching orders near ${latitude}, ${longitude} within ${range}km`
    );
    ...
    console.log(
      `Found ${transportOrders.length} transport orders and ${parcelOrders.length} parcel orders`
    );
...
router.post("/orders/:id/reached-destination", auth, async (req, res) => {
  try {
    ...
    const validStatus =
      modelType === "transport" ? "in_progress" : "in_transit";
...
router.post("/orders/:id/cancel", auth, async (req, res) => {
  try {
    ...
    const completedStatus =
      modelType === "transport" ? "completed" : "delivered";
  ...
  try {
    ...
    const [transportEarnings, parcelEarnings] = await Promise.all([
      Transport.find({
        ...
        .select("fareEstimate completedAt")
        .lean(),
...
router.get("/earnings/summary", auth, async (req, res) => {
  try {
    ...
    // Calculate totals
    const todayTotal =
      (todayEarnings[0][0]?.total || 0) + (todayEarnings[1][0]?.total || 0);
    const weekTotal =
      (weekEarnings[0][0]?.total || 0) + (weekEarnings[1][0]?.total || 0);
    const monthTotal =
      (monthEarnings[0][0]?.total || 0) + (monthEarnings[1][0]?.total || 0);
    const totalAmount =
      (totalEarnings[0][0]?.total || 0) + (totalEarnings[1][0]?.total || 0);
