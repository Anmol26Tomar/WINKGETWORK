...
const DEFAULT_REGION = { latitude: 28.6139, longitude: 77.2090, latitudeDelta: 0.08, longitudeDelta: 0.08 };
...
export default function AllIndiaParcelScreen() {
	...
	// Search state (like truck-booking-form)
	...
	const [pkgType, setPkgType] = useState('Documents');
	...
	useEffect(() => {
		...
		setDistanceKm(km);
		setFare(estimateFareKm(km, 'truck'));
	}, [pickup, delivery]);

	const handleSearchChange = handleSearchChangeFactory({ setPickupSearch, setDeliverySearch, searchTimeoutRef, setSearchResults, setShowResults });
	const handleAddressSelect = handleAddressSelectFactory({ setPickup, setDelivery, setPickupSearch, setDeliverySearch, setShowResults, mapRef });
	const handleAddressSubmit = handleAddressSubmitFactory({ setPickup, setDelivery, setPickupSearch, setDeliverySearch, mapRef });
	const onMapPress = handleMapPressFactory({ pickup, setPickup, setPickupSearch, setDelivery, setDeliverySearch });

	const onSubmit = async () => {
		if (!pickup || !delivery || !pkgType || !weight || !receiverName || !receiverPhone) {
		...
		try {
			...
			const payload = {
				...
				package: { name: pkgType, size: dimensions || 'standard', weight: weightNum, description: '' },
				receiverName,
	...
	return (
		<View style={styles.container}>
			...
			<ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
				...
				<Text style={styles.meta}>Choose pickup and delivery via map or search.</Text>
...
// Helpers adapted from truck-booking-form.jsx
