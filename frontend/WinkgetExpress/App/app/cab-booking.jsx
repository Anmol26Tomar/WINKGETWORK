...
export default function CabBookingScreen() {
	...
	useEffect(() => {
		const run = async () => {
			...
			try {
				setLoading(true);
				const res = await estimateTransport({ pickup, destination, vehicleType: 'cab' });
				setDistanceKm(res.distanceKm);
				setFare(res.fare);
			} catch {
				const km = haversineKmLocal(pickup, destination);
				setDistanceKm(km);
				setFare(estimateFareKmLocal(km, 'cab'));
	...
	const onSubmit = async () => {
		...
		try {
			setLoading(true);
			await createTransport({ pickup, destination, vehicleType: 'cab' });
			Alert.alert('Booked', `Your cab is on the way. Est. fare ₹${fare}`);
